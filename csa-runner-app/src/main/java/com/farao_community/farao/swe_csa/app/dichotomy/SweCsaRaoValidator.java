...
public class SweCsaRaoValidator {
    ...
    public DichotomyStepResult validateNetworkForPortugueseBorder(Network network, Crac crac,
        String cracUri, ZonalData<Scalable> scalableZonalData, RaoParameters raoParameters,
        CsaRequest csaRequest, String raoParametersUrl, CounterTradingValues counterTradingValues) {
        return validateNetworkForBorder(network, crac, cracUri, csaRequest, raoParametersUrl,
            counterTradingValues, "PT-ES", scalableZonalData, raoParameters);
    }

    @WithSpan("validateNetworkForFrenchBorder")
    public DichotomyStepResult validateNetworkForFrenchBorder(Network network, Crac crac,
        String cracUri, ZonalData<Scalable> scalableZonalData, RaoParameters raoParameters,
        CsaRequest csaRequest, String raoParametersUrl, CounterTradingValues counterTradingValues) {
        return validateNetworkForBorder(network, crac, cracUri, csaRequest, raoParametersUrl,
            counterTradingValues, "FR-ES", scalableZonalData, raoParameters);
    }

    private DichotomyStepResult validateNetworkForBorder(Network network, Crac crac,
        @SpanAttribute("cracUri") String cracUri, CsaRequest csaRequest,
        @SpanAttribute("raoParametersUrl") String raoParametersUrl,
        CounterTradingValues counterTradingValues, @SpanAttribute("border") String border,
        ZonalData<Scalable> scalableZonalDataFilteredForSweCountries, RaoParameters raoParameters) {
        RaoRequest raoRequest = buildRaoRequest(counterTradingValues.print(),
            csaRequest.getBusinessTimestamp(), csaRequest.getId(), network, cracUri,
            raoParametersUrl, border);
        ...
        try {
            ...
            if (abstractRaoResponse.isRaoFailed()) {
                ...
                businessLogger.error("[{}] : RAO computation failed: {}", border,
                    raoFailureResponse.getErrorMessage());
            ...
            if (raoSuccessResponse.isInterrupted()) {
                throw new RaoInterruptionException(String.format(
                    "[%s] : RAO computation related to CSA task: [%s], stopped due to interruption request",
                    raoRequest.getId(), border));
            }
            RaoResult raoResult = new RaoResultJsonImporter().importData(
                new URI(raoSuccessResponse.getRaoResultFileUrl()).toURL().openStream(), crac);
            ...
            if (isSecure && !crac.getAngleCnecs().isEmpty()) {
                businessLogger.info("{} crac contains Angle CNECs. Angle monitoring will be run.",
                    border);
                raoResult = resultHelper.updateRaoResultWithAngleMonitoring(network, crac,
                    scalableZonalDataFilteredForSweCountries, raoResult, raoParameters);
                isSecure = raoResult.isSecure(PhysicalParameter.FLOW, PhysicalParameter.ANGLE);
                if (isSecure) {
                    businessLogger.info(
                        "Angle monitoring secure for {} border, Final result will contain Angle monitoring results",
                        border);
                } else {
            ...
            if (isSecure && !crac.getVoltageCnecs().isEmpty()) {
                businessLogger.info(
                    "{} crac contains Voltage CNECs. Voltage monitoring will be run.", border);
                raoResult = resultHelper.updateRaoResultWithVoltageMonitoring(network, crac,
                    raoResult, raoParameters);
                isSecure = raoResult.isSecure(PhysicalParameter.FLOW, PhysicalParameter.VOLTAGE);
                if (isSecure) {
                    businessLogger.info(
                        "Voltage monitoring secure for {} border, Final result will contain Voltage monitoring results",
                        border);
                } else {
            ...
            return DichotomyStepResult.fromNetworkValidationResult(raoResult, isSecure,
                raoSuccessResponse, counterTradingValues);
        } catch (Exception e) {
    ...
    private void logBorderOverload(RaoResult raoResult, Crac crac, String borderName) {
        if (raoResult.isSecure(PhysicalParameter.FLOW)) {
            ...
            businessLogger.info("There is overloads on '{}' border, network is not secure",
                borderName);
            Set<FlowCnec> flowCnecs = getBorderFlowCnecs(crac, borderName);
            Pair<String, Double> flowCnecSmallestMargin = getFlowCnecSmallestMargin(raoResult,
                flowCnecs);
            businessLogger.info(
                "On the '{}' border, the most limiting CNEC is {}, with a margin of {}", borderName,
                flowCnecSmallestMargin.getLeft(), flowCnecSmallestMargin.getRight());
    ...
    Pair<String, Double> getFlowCnecSmallestMargin(RaoResult raoResult, Set<FlowCnec> flowCnecs) {
        ...
        for (FlowCnec flowCnec : flowCnecs) {
            double margin = raoResult.getMargin(flowCnec.getState().getInstant(), flowCnec,
                Unit.AMPERE);
    ...
    private RaoRequest buildRaoRequest(@SpanAttribute("stepFolder") String stepFolder,
        @SpanAttribute("timestamp") String timestamp, @SpanAttribute("taskId") String taskId,
        Network network, @SpanAttribute("cracUrl") String cracUrl,
        @SpanAttribute("raoParametersUrl") String raoParametersUrl,
        @SpanAttribute("border") String border) {
        String scaledNetworkPath = generateScaledNetworkPath(network, timestamp, stepFolder);
        String scaledNetworkPreSignedUrl = fileExporter.saveNetworkInArtifact(taskId, network,
            scaledNetworkPath);
    ...
    private String generateArtifactsFolder(String timestamp, String stepFolder) {
        ...
        return "artifacts" + "/" + offsetDateTime.getYear() + "/" + offsetDateTime.getMonthValue()
            ...
            + offsetDateTime.getMinute() + "/" + stepFolder;
    }
    ...
    private String generateScaledNetworkPath(Network network, String timestamp, String stepFolder) {
        return generateArtifactsFolder(timestamp, stepFolder) + "/" + "network-"
            + network.getVariantManager().getWorkingVariantId() + ".xiidm";
    }
